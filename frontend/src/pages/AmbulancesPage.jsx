...
export default function AmbulancesPage() {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const newAmbulances = ambulances.map((i) =>
  ...
  const fetchHospitalById = async (hospitalId) => {
    try {
      ...
      const hospitals = await get_hospitals();
  ...
  return (
    <div className="app-container">
              ...
              <div key={ambulance.id} className="ambulances-card">
                {userRole === "admin" && (
                  <div className="CRUD-buttons">
                    <button onClick={() => openEditModal(ambulance)}>
                      <FaEdit />
                    </button>
                    <button onClick={() => openDeleteModal(ambulance)}>
                      <FaTrash />
                    </button>
                  </div>
                )}
                ...
                <p>
                  Location: ({ambulance.lat?.toFixed(4)},{" "}
                  {ambulance.lon?.toFixed(4)})
                </p>
                <p>
                  Base: ({ambulance.default_lat?.toFixed(4)},{" "}
                  {ambulance.default_lon?.toFixed(4)})
                </p>
      ...
      >
        <p>
          Are you sure you want to delete Ambulance #{selectedAmbulance?.id}?
        </p>
        ...
        <form>
          <div className="form-group">
            ...
            <input
              ...
              name="base_hospital_id"
              placeholder="e.g. 1"
              value={formData.base_hospital_id || ""}
              onChange={handleInputChange}
            />
          </div>
